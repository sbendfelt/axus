...
class TestSupport {
  ...
  setRunAsScope(runAsScope) {
    this.contextBuilder.setRunAsScope(runAsScope);
    return this;
  }
...
function requireRest(resource, name, additionalContext, apiVersion, dependsOn, runAsScope) {
    ...
    if (runAsScope) {
      ts.setRunAsScope(runAsScope);
    }
    const runnable = ts.useRest();
    return r(name, runnable);
}

function requireLocal(resource, name, additionalContext, apiVersion, dependsOn, runAsScope) {
    const ts = new TestSupport(resource).addToContext(additionalContext);
    if (apiVersion) {
      ts.useApiVersion(apiVersion);
    }
