...
class PersistenceProvider {
  ...
  createFetchRequest() {
    if (!arguments.length || arguments.length === 1) {
      ...
      throw new TypeError('Unsupported parameters used in createFetchRequest function. Please check persistenceProvider API.');
    }
  }
  ...
  execute(request) {
    if(!request.execute) {
      throw new TypeError('Request cannot be executed. Please check that this is actually a request.');
    }
