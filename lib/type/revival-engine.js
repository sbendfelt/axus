...
function reviveAppXObject(o) {
    ...
    return transform(o, root, appxType.design);
}
 ...
 */
function transform(source, dictScope, design) {
    const dictionary = design.DataDictionary;
    const destination = {};
    Object
        .keys(source)
        .forEach((k) => {
            ...
            if (!typeDesc && isCustomObject(dictionary)) {
                const auxilliaryData = getAuxilliaryFieldData(design, k);
                if(auxilliaryData) {
                  const root = auxilliaryData.root;
                  const newDesign = auxilliaryData.design;
                  transform(val, root, newDesign);
                  return;
                }
            }
            if (!typeDesc) {
            ...
            if (typeDesc.type === 'TEXT') {
                ...
                destination[k] = transformCollection(val, nextDictScope, design);
            } else if (typeDesc.isMap) {
                destination[k] = transformMap(val, nextDictScope, design);
            } else {
                destination[k] = transform(val, nextDictScope, design);
            }
        });
    return destination;
}
 ...
 * @param  {object} target     the object that we want to transform
 * @param  {object} dictScope  the dictionary entry for the current target
 * @param  {object} design     the entire design
 */
function transformCollection(sources, dictScope, design) {
    return sources.map((source) => {
        return transform(source, dictScope, design);
    });
}
 ...
 * @param  {object} dictionary the entire dictionary from the design.
 */
function transformMap(target, dictScope, design) {
    if (isSimpleKVMap(dictScope)) {
        return transform(target, makeKVDict(target, dictScope.$value), design);
    } else if (isIndirectMap(target, dictScope)) {
        return transformIndirectMap(target, dictScope, design);
    } else {
        return transform(target, dictScope, design);
    }
}

function transformIndirectMap(target, dictScope, design) {
    return Object
        .keys(target)
        .reduce((acc, v) => {
            const mapEntry = target[v];
            if (Array.isArray(mapEntry)) {
                acc[v] = transformCollection(mapEntry, dictScope, design);
            } else {
                acc[v] = transform(mapEntry, dictScope, design);
            }
            return acc;
        }, {});
}
...
//TODO: this might not be the smartest impl. could also just use design
function isCustomObject(dictionary) {
    return dictionary.type.startsWith('$');
}

//TODO: break this down. remove dependency on v310
function getAuxilliaryFieldData(design, key) {
    const fieldData = design.design && design.design.fieldData;
    ...
    const dd = typeDef.design.DataDictionary;
    const root = dd[fieldDataEntry.type] || dd[initCap(fieldDataEntry.type)];
    return root ? {design: typeDef.design, root: root} : null;
}
